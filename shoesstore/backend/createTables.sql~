...
VALUES ('admin', '$2a$04$f6MJKU3x50kcIx5kTQdOf.VYwWUZQgSR4xld82QbDRzROAe5v.Uoe', 'Administrador Principal', 'ROLE_ADMIN');
...
BEGIN
  ...
  WHILE i <= 100 DO
    ...
    -- Asignar un usuario SELLER aleatorio (IDs 2, 3 o 4)
...
BEGIN
  ...
  read_loop: LOOP
    ...
    detail_loop: WHILE j <= num_details DO
      ...
      VALUES (
        current_sale_id,
        FLOOR(1 + (RAND() * 50)),   -- Selecciona un producto aleatorio (ID entre 1 y 50)
        FLOOR(1 + (RAND() * 5)),    -- Cantidad aleatoria entre 1 y 5
        ROUND(10 + (RAND() * 90), 2)  -- Subtotal aleatorio entre 10 y 100
      );
      SET j = j + 1;
